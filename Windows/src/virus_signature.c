...
// #include <string.h>
// #include <pthread.h>
// #include <openssl/md5.h>  // Ensure OpenSSL is installed and linked
...
//     const int buffer_size = 32768;  // 32 KB buffer
//     unsigned char *buffer = malloc(buffer_size);
//     if (!buffer) {
//         perror("Failed to allocate memory for hashing");
//         fclose(file);
//         exit(EXIT_FAILURE);
//     }
...
// void *compare_hashes(void *args) {
//     ThreadArgs *thread_args = (ThreadArgs *)args;

//     for (int i = thread_args->start_idx; i < thread_args->end_idx; i++) {
//         if (*thread_args->found) {
//             pthread_exit(NULL);  // Exit if a match has already been found
//         }
...
//         pthread_create(&threads[i], NULL, compare_hashes, &thread_args[i]);
//     }
...
DWORD WINAPI compare_hashes(LPVOID args) {
    ...
    for (int i = thread_args->start_idx; i < thread_args->end_idx; i++) {
        if (*thread_args->found) return 0;  // Exit if a match has already been found
